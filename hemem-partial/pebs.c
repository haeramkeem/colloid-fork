...
static void colloid_setup(int cpu) {
  ...
  for(cha = 0; cha < NUM_CHA_BOXES; cha++) {
	// Icelake offset multiplier is 0xE
      ...
      /*
       * 00c81686
       * CHA Counter Control Register
       * 0xHH[31:24]: Threshold
       *   0x00: (NULL)
       * 0xH[23:20]: (Flag 1 - invert(23), enable(22), reserved(21), overflow_enable(20))
       *   0x4: enable
       * 0xH[19:16]: (Flag 0 - tid_en(19), edge_detect(18), reset(17), reserved(16))
       *   0x0: (NULL)
       * 0xHH[15:8]: Unit mask
       *   0x01: IRQ
       * 0xHH[0:7]: Event select
       *   0x36: TOR Occupancy
       *   0x35: TOR Inserts
       */
...
static void colloid_update_stats() {
  ...
  sample_cha_ctr(0, 0); // CHA0 (Local) occupancy (O_D)
  sample_cha_ctr(0, 1); // CHA0 (Local) inserts (R_D)
  sample_cha_ctr(1, 0); // CHA1 (Remote) occupancy (O_A)
  sample_cha_ctr(1, 1); // CHA1 (Remote) inserts (R_A)
